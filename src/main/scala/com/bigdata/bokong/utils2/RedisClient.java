...
public class RedisClient {
    ...
    static {
        ...
        jedisClusterNodes.add(new HostAndPort("10.20.4.7", 7000));
        jedisClusterNodes.add(new HostAndPort("10.20.4.7", 7001));
        jedisClusterNodes.add(new HostAndPort("10.20.4.7", 7002));
        jedisClusterNodes.add(new HostAndPort("10.20.4.7", 7003));
        jedisClusterNodes.add(new HostAndPort("10.20.4.7", 7004));
        jedisClusterNodes.add(new HostAndPort("10.20.4.7", 7005));
    ...
    private void initialShardedPool() {
        ...
        // 一个jedis实例时，是否进行alidate操作，如果为true,则得到的jedis实例都是可用的
        config.setTestOnBorrow(false);//jedis第一次使用时会报错
        // slave链接
        //redis.port=6379
    ...
    public void test1(){
         ...
         * 更新，返回值为0
         */
        long n = redisClient.getJedisCluster().hset("hsetkey", "hashKey1", "hashValue22244333466a");//
        System.out.println("n1 : "+n);
        String hash2 = redisClient.getJedisCluster().hget("hsetkey", "hashKey1");
        System.out.println("测试 hset hget ： hsetkey 的返回值："+hash2);
        n = redisClient.getJedisCluster().hset("hsetkey", "hashKey1", "hashValue777b");//
        System.out.println("n2 : "+n);
        String hash = redisClient.getJedisCluster().hget("hsetkey", "hashKey1");//返回哈希表key 中给定域field 的值
        System.out.println("测试 hset hget ： hsetkey 的返回值："+hash);
//        redisClient.getJedisCluster().set("set1","value1");
    }
    @Test
    public void test2(){
        /**
         * hsetnx 将哈希表 key 中的域 field(指第二个参数) 的值设置为 value ，当且仅当域 field 不存在。
         * 若域 field(指第二个参数) 已经存在，返回值为0，该操作无效。
         * 失败，返回值为0
         * 成功，返回值为不等于0
         */
//        long n = redisClient.getJedisCluster().hsetnx("hsetkeynx", "hashkeynx", "hashvaluenx");
//        System.out.println(n!=0?"操作成功":"操作失败"); // print 操作成功
//        n = redisClient.getJedisCluster().hsetnx("hsetkeynx", "hashkey", "hashvaluenx");
//        System.out.println(n!=0?"操作成功":"操作失败"); // print 操作成功
//        n = redisClient.getJedisCluster().hsetnx("hsetkeynx", "hashkey", "hashvaluenx");
//        System.out.println(n!=0?"操作成功":"操作失败"); // print 操作失败

        Map<String,String> hashMapKey = redisClient.getJedisCluster().hgetAll("VIDEO_CONT_WAIT_20180103");
    ...
    public void test3(){
        ...
        hashMap.put("hashMap11", "hashValue11");
        hashMap.put("hashMap21", "hashValue21");
        hashMap.put("hashMap31", "hashValue31");
        hashMap.put("hashMap41", "hashValue41");
        String status  = redisClient.getJedisCluster().hmset("hashMapkey", hashMap);//如果命令执行成功，返回OK 。当key 不是哈希表(hash) 类型时，返回一个错误。
        String hash = redisClient.getJedisCluster().hget("hashMapkey", "hashMap46");
        System.out.println("OK".equals(status)?"操作成功  返回值："+hash:"操作失败");
        //返回值： 一个包含多个给定域的关联值的表，表值的排列顺序和给定域参数的请求顺序一样
        List<String> hashList = redisClient.getJedisCluster().hmget("hashMapkey", "hashMap1 hashMap2 hashMap3 hashMap4".split(" "));
        ...
        Map<String,String> hashMapKey = redisClient.getJedisCluster().hgetAll("hashMapkey");
        System.out.println(hashMapKey);
        // map 的第一种迭代方式
    ...
    public void test31(){
         ...
         *
         */
//        String hash = redisClient.getJedisCluster().hget("hashMapkey", "hashMap4");
//        System.out.println("OK".equals(hash)?"操作成功  返回值："+hash:"操作失败");
        //返回值： 一个包含多个给定域的关联值的表，表值的排列顺序和给定域参数的请求顺序一样

        long status = redisClient.getJedisCluster().hdel("hashMapkey","hashMap11");
        System.out.println(status);
//        List<String> hashList = redisClient.getJedisCluster().hmget("hashMapkey", "hashMap1".split(" "));
//        for(String value : hashList){
//            System.out.println("对应的value值：  "+value+" ");//返回值： 一个包含多个给定域的关联值的表，表值的排列顺序和给定域参数的请求顺序一样
//        }
    }
    ...
    @Test
    public void test5(){
        //hkeys keys 返回 所有的key  ,hkeys 返回 key 下面的所有的 域
        Set<String> setKey = redisClient.getJedisCluster().hkeys("VIDEO_CONT_WAIT_20180117");
...
//        System.out.println(redisClient.getJedisCluster().hexists("wait_20171229","10005_0"));
    }
    @Test
    public void test6(){
...
//        System.out.println("对 hash表中key 为hashMapkey 的域hashMap1 的值  减去 1 结果为："+n);// 返回值：对 hash表中key 为hashMapkey 的域hashMap1 的值  减去 1 结果为：0
    ...
    public void testlist1(){
         ...
         */
        long n = redisClient.getJedisCluster().lpush("jedisList", "a");
        List<String> jedisList = redisClient.getJedisCluster().lrange("jedisList", 0, -1);
        System.out.println("jedisList 返回列表长度： "+ n +" 列表的值： "+jedisList);
        n = redisClient.getJedisCluster().lpush("jedisList", "a b".split(" "));// 列表允许值有重复
        jedisList = redisClient.getJedisCluster().lrange("jedisList", 0, 16);
        System.out.println("jedisList 返回列表长度： "+ n +" 列表的值： "+jedisList);
    }
//    @Test
...
//    }
    ...
    public void testset3(){
        //llen返回列表key 的长度。 如果key 不存在，则key 被解释为一个空列表，返回0 . 如果key 不是列表类型，返回一个错误。
        long n = redisClient.getJedisCluster().sadd("jedisset1","a");
        Boolean isExist = redisClient.getJedisCluster().sismember("jedisset1","a");
        System.out.println("=1=="+n + redisClient.getJedisCluster().smembers("jedisset1")+",isExist:"+isExist);
        n = redisClient.getJedisCluster().sadd("jedisset1","a b c".split(" "));
        System.out.println("=2=="+n + redisClient.getJedisCluster().smembers("jedisset1"));
    }
}