  ...
  object joinTest extends App {
    ...
    val res1 = student_all.mapPartitions(iter => {
      ...
      for {
        ...
        if (stuMap.contains(idCard))
      } yield (idCard, stuMap.getOrElse(idCard, "null"), school)
    })

    res.foreach(println)

    //((01000082123,其他),(1,None))
sc.textFile("file:///E:\\MyProject\\bc-kong\\data\\check2\\part-00000").map(_.split(",")(1).replace(")",""))
      .map((_,1)).reduceByKey(_+_,1).saveAsTextFile("file:///E:\\MyProject\\bc-kong\\data\\check4")
  }